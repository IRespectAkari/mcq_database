const charaData = [
  ...
  {
    name: "チュチュ",
    ability: "アイテム転職 その他 剣〇、杖✕",
    skills: ["クリティカル","燃焼(毎ターンHP15%消費＆2回攻撃。効果は自分のみ)","力がみなぎる(攻撃力UP)",]
  },
  {
    name: "プラグ",
    ability: "アイテム転職 探索 剣〇、杖✕",
    skills: ["クリティカル","ドローン召喚(毎ターン確定追加ダメージ【攻撃力は本体に依存】)","探索技能","幸運","金運",]
  },
  {
    name: "クリーミー",
    ability: "アイテム転職 援護 剣〇、杖〇",
    skills: ["クリームトラップ(確率で敵に「攻撃力DOWN」「防御力DOWN」「3ターン行動不能」のどれかを付与)","HP回復","敵バリア軽減","呪い無効化","状態異常治療",]
  },
  {
    name: "アイアン",
  ...
  },
  {
    name: "クッカー",
    ability: "アイテム転職 その他 剣〇、杖〇",
    skills: ["所持しているレタスで料理を作り仲間を呼ぶ。","レッドレタス所持　 戦士系","イエローレタス所持 援護系","ブラックレタス所持 特殊系","ゴールドレタス所持 探索系","シロガネレタス所持 その他系","※複数持っている場合、ランダムで選ばれる。","※アイテムは消費しない。","冒険開始時、料理を振舞い活動ターン数を伸ばす。"]
  },
  {
    name: "ババ",
    ability: "アイテム転職 その他 剣◎、杖✕",
    skills: ["クリティカル(防御を無視したダメージ)","ハンマー(戦闘開始時クリティカル無効解除)","金槌(毎ターン確率で敵防御力DOWN)",]
  },
  {
    name: "マッシュ",
    ability: "アイテム転職 特殊 剣✕、杖◎",
    skills: ["先制攻撃","文字解読","敵の能力発動率減少(※？)","敵クリティカル率減少※","ガス(敵を盲目状態にする)※","※自分攻撃した後に確率で発動",]
  },
];
...
function createSelectorOptions() {
  charaData.sort(kanaSort).forEach(e=>charaSelector.appendChild(createAndSetT(`option`, e.name)));
}
...
function kanaSort(a, b) {
  let c = a.name.charCodeAt(0) - b.name.charCodeAt(0);
  return c !== 0 ? c : a.name.charCodeAt(1) - b.name.charCodeAt(1);
}
